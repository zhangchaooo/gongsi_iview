...
export default {
  ...
  methods: {
    ...
    getData () {
      this.loading = true
      staff(this.form.id).then(res => {
        this.loading = false
        if (res) {
          this.form = res
          this.form.tissue_id = getValueArray(res.tissue, 'id')
          this.editDep = res.tissue
          // this.$refs.depTree.setData(getValueArray(res.tissue, 'name').join(','))
          this.form.image_url && this.$refs.uploadThumb.setData(this.form.image_url)
        }
      })
    },
    handleSubmit () {
      this.$refs.form.validate(valid => {
        if (valid) {
          staffEdit(this.form.id, this.form).then(res => {
