            ...
            <li v-for="(item, index) in dataHusing"
              ...
              <span style="vertical-align: middle;">
                <span style="display: inline-block;">
                  <i class="ivu-icon"
                     :class="listType == 3 ? 'ivu-icon-md-home' : (listType == 2 ? 'ivu-icon-md-list' : 'ivu-icon-ios-apps')"
...
import { getHousingTree, doorHouseTree } from '@/api/community/housing'
import { staffList } from '@/api/global/staff'
export default {
  name: 'housingTreeChoose',
  props: {
    multiple: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    ModalOpen () {
      ...
      } else { this.dataHusing = [] }
      this.listType > 3 ? this.initUserData() : this.initData()
      this.searchKey = ''
      this.searchData = []
    ...
    cancel () {
      ...
      if (this.listType < 3) this.dataHusing = []
    ...
    initData () {
      doorHouseTree(this.housingId).then(res => {
        if (res) {
          if (this.listType == 3) {
    ...
    selectTree (v) {
      ...
      if (!this.multiple) this.checkPermTree(this.data, this.dataHusing, 0)
      if (v.selected) {
        this.multiple ? this.dataHusing.push({ 'id': v.id, 'alias': v.alias }) : this.dataHusing = [{ 'id': v.id, 'alias': v.alias }]
      } else {
        this.multiple ? this.dataHusing.splice(this.dataHusing.findIndex(item => item.id === v.id), 1) : this.dataHusing = []
      }
      // 选中后重新生成一维搜索树
