<template>
  <div>
    <Button v-for="(item, index) in dataDepView"
            :key="index"
            :icon="item.phone_number ? 'md-person' : 'ios-folder'"
            class="mr-5">{{item.name}}</Button>

    <Button @click="ModalOpen"
            icon="ios-create-outline">设置</Button>
              ...
              <li v-for="(item, index) in dataDep"
                ...
                <span style="vertical-align: middle;">
                  <span style="display: inline-block;">
                    <i class="ivu-icon primary-color"
                       :class="item.phone_number ? 'ivu-icon-md-person' : 'ivu-icon-ios-folder'"
                       style="font-size: 16px; margin-right: 8px; margin-bottom: 3px;"></i>
                  ...
                  <Icon @click="deleteDataDep(index, item.id)"
                        class="primary-color"
                        style="cursor: pointer; font-size: 26px; line-height: 24px;float: right;"
                        type="ios-close" />
...
export default {
  ...
  watch: {
    editDep (val) {
      // 防止脏数据，至少选择了一个，并且按规则是有key的，才进行编辑已选择数据的渲染
      if (val[0].key) {
        this.dataDepView = val
        // console.log(val)
        this.dataDep = val.map(i => {
          return { ...i, key: i.id + i.name + i.phone_number }
        })
      }
    }
  },
  methods: {
    ...
    submit () {
      ...
      this.listType === 2
        ? this.$emit('on-change', this.dataDep)
        : this.$emit('on-change', getValueArray(this.dataDep, 'id'))
      this.dataDepView = JSON.parse(JSON.stringify(this.dataDep))
      this.searchKey = ''
      this.searchData = []
      this.ModalVisible = false
    },
    ...
    selectSearchTree (v) {
      if (getValueArray(this.dataDep, 'key').includes(v.id + v.name + v.phone_number)) {
        ...
        this.multiple
          ? this.dataDep.push({ 'id': v.id, 'name': v.name, 'phone_number': v.phone_number, 'key': v.id + v.name + v.phone_number })
          : this.dataDep = [{ 'id': v.id, 'name': v.name, 'phone_number': v.phone_number, 'key': v.id + v.name + v.phone_number }]
      }
      // 模仿企业微信，拿完数据就把搜索的树清空
      this.clearSearchDep()
    },
    selectTree (v) {
      if (this.listType === 1 && !v.phone_number) {
  ...
  mounted () {
    ...
    // // 编辑的话需要把传进来的数据组装一下 判断选择key
    // if (this.editDep) {
    //   this.dataDepView = this.editDep
    //   this.dataDep = this.editDep.map(i => {
    //     return { ...i, key: i.id + i.name + i.phone_number }
    //   })
    // }
  }
